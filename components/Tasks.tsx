    ...
    <div className="flex-1 p-6 lg:p-10 max-w-[1400px] mx-auto w-full space-y-8 overflow-y-auto pb-32">
      ...
      <header className="flex justify-between items-start">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            onClick={onNewTask}
            className="bg-primary hover:bg-blue-400 text-black font-black px-6 py-3 rounded-xl flex items-center gap-2 shadow-lg shadow-primary/20 transition-all active:scale-95"
          >
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Tarefas Pendentes', value: pendingTasks.length, icon: 'calendar_today', color: 'blue', tag: 'Hoje' },
          { label: 'Tarefas Iniciadas', value: '12', icon: 'more_horiz', color: 'yellow', tag: 'Em Andamento' },
          { label: 'Tarefas Concluídas', value: completedTasks.length, icon: 'check_circle', color: 'green', tag: '+12%', isPill: true },
          { label: 'Tempo de Foco', value: '3h 45m', icon: 'timer', color: 'purple' },
        ].map((stat, i) => (
          <div key={i} className="bg-surface-dark/50 p-5 rounded-2xl border border-white/5 relative overflow-hidden group hover:border-white/10 transition-all">
            <div className="flex justify-between items-start mb-4">
              ...
              {stat.tag && (
                <span className={`text-[9px] font-black px-2 py-1 rounded ${stat.isPill ? 'bg-primary/10 text-primary' : 'bg-white/5 text-slate-500'} uppercase tracking-widest`}>
                  {stat.tag}
                </span>
              )}
            </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-center">
        ...
        <div className="flex items-center gap-2 overflow-x-auto w-full lg:w-auto pb-2 lg:pb-0 scrollbar-hide">
          {[
            { label: 'Hoje', count: 5 },
            { label: 'Próximos 7 dias', count: 0 },
            { label: 'Alta Prioridade', count: 0, isDot: true },
            { label: 'Atrasado', count: 0, icon: 'warning' }
          ].map((f) => (
            <button 
              key={f.label}
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center">
             <span className="material-symbols-outlined text-primary text-[14px] icon-fill">play_arrow</span>
          </div>
          <h3 className="text-sm font-black text-white uppercase tracking-widest">Em Progresso</h3>
        </div>
          ...
          {pendingTasks.map((task) => (
            <div key={task.id} className="group relative bg-surface-dark/30 hover:bg-surface-dark/50 p-4 rounded-xl border border-white/5 flex items-center gap-4 transition-all cursor-pointer">
              ...
              <div className="flex items-center gap-4 shrink-0">
                <span className="bg-blue-500/10 text-blue-400 text-[10px] font-black px-2.5 py-1 rounded-md uppercase tracking-wider border border-blue-500/10">
                  {task.category}
                </span>
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/5 border border-white/5">
                  <span className="material-symbols-outlined text-slate-500 text-[14px]">calendar_month</span>
                  <span className={`text-[10px] font-black uppercase ${task.dueDate === 'Hoje' ? 'text-orange-400' : 'text-slate-400'}`}>
                    {task.dueDate}
                  </span>
                </div>
                ...
                <div className="w-8 h-8 rounded-lg border border-white/10 overflow-hidden bg-slate-800 flex items-center justify-center">
                   {task.id === '1' ? (
                      ...
                      <span className="text-[10px] font-black text-slate-400">EU</span>
                   )}
                </div>
              </div>
            </div>
      ...
      <section className="space-y-4 pt-4">
        <div className="flex items-center gap-3">
          <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center">
             <span className="material-symbols-outlined text-primary text-[14px] icon-fill">check</span>
          </div>
          <h3 className="text-sm font-black text-white uppercase tracking-widest">Concluídas Hoje</h3>
        </div>
          ...
          {completedTasks.length === 0 ? (
            ...
            completedTasks.map((task) => (
              <div key={task.id} className="group bg-surface-dark/10 p-4 rounded-xl border border-white/5 flex items-center gap-4 transition-all">
                <button 
                  onClick={() => onToggle(task.id)}
                  className="w-6 h-6 rounded-lg bg-primary border-2 border-primary flex items-center justify-center text-black shrink-0"
                >
                ...
                <div className="flex items-center gap-4">
                   <span className="bg-slate-800 text-slate-500 text-[10px] font-black px-2.5 py-1 rounded-md uppercase border border-white/5">
                    {task.category}
                  </span>
                  <div className="w-8 h-8 rounded-lg border border-white/10 bg-slate-900 flex items-center justify-center">
                     <span className="text-[10px] font-black text-slate-600">EU</span>
                  </div>
                </div>
              </div>
            ))
