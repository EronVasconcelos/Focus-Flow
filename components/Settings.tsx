...
const Settings: React.FC<SettingsProps> = ({ onSave }) => {
  return (
    <div className="flex-1 p-6 lg:p-10 max-w-[900px] mx-auto w-full space-y-10 overflow-y-auto">
      <header>
        <h1 className="text-3xl lg:text-4xl font-bold text-slate-900 dark:text-white tracking-tight">Configurações</h1>
      ...
      <section className="bg-white dark:bg-surface-dark p-8 rounded-3xl border border-white/5 shadow-2xl space-y-8">
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="text-center md:text-left">
            <h2 className="text-2xl font-bold dark:text-white">Alex Silva</h2>
            <p className="text-slate-500 dark:text-[#9db9a1] font-medium">alex.design@pro.com</p>
            <div className="mt-3 flex flex-wrap gap-2 justify-center md:justify-start">
              <span className="bg-primary/10 text-primary text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-widest border border-primary/20">Mestre Produtividade</span>
              <span className="bg-blue-500/10 text-blue-500 text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-widest border border-blue-500/20">Plano Pro</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={pref.id} className="flex items-center justify-between p-5 bg-white dark:bg-surface-dark rounded-2xl border border-white/5 hover:border-primary/20 transition-all cursor-pointer">
              <div>
                <p className="font-bold text-sm dark:text-white">{pref.label}</p>
      ...
      <div className="flex justify-end gap-3 pb-20">
        ...
        <button onClick={onSave} className="px-8 py-4 rounded-2xl bg-primary text-black font-bold text-sm shadow-xl shadow-primary/20 hover:scale-[1.05] active:scale-95 transition-all">Salvar Alterações</button>
      </div>
    </div>
  );
};
