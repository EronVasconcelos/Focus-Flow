  ...
  return (
    <div className="flex-1 p-6 lg:p-10 max-w-[1600px] mx-auto w-full space-y-8 overflow-y-auto pb-32">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black text-white tracking-tight">Metas de Longo Prazo</h1>
          <p className="text-slate-400 font-medium mt-1">Divida grandes sonhos em etapas e transforme objetivos em conquistas.</p>
        </div>
      ...
      <div className="flex bg-white/5 p-1 rounded-2xl border border-white/5 w-fit">
        <button 
          ...
          className={`px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'longo-prazo' ? 'bg-background-dark text-white border border-white/5 shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}
        >
          Longo Prazo
        </button>
        <button 
          onClick={() => setActiveTab('diarias')}
          className={`px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'diarias' ? 'bg-background-dark text-white border border-white/5 shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}
        >
          Metas Diárias
        </button>
      </div>
      ...
      {activeTab === 'longo-prazo' ? (
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {goals.map((goal) => (
            <div key={goal.id} className="bg-surface-dark/40 border border-white/5 rounded-[2.5rem] p-8 space-y-8 group hover:border-white/10 transition-all relative overflow-hidden">
               <div className="flex justify-between items-start relative z-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] font-black text-primary bg-primary/10 px-2.5 py-1 rounded-lg uppercase tracking-widest mt-2 inline-block">
                        {goal.category}
                      </span>
                    </div>
                  </div>
               ...
               <div className="space-y-4">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Próximas Etapas</p>
                    ...
                    {goal.steps.map((step, idx) => (
                      <div key={idx} className="flex items-center gap-4 p-4 bg-white/[0.02] border border-transparent hover:border-white/5 rounded-2xl transition-all group/step cursor-pointer">
                        <div className={`w-6 h-6 rounded-lg flex items-center justify-center border-2 transition-all ${step.completed ? 'bg-primary border-primary text-black' : 'border-slate-700 group-hover/step:border-primary'}`}>
                          {step.completed && <span className="material-symbols-outlined text-[14px] font-black">check</span>}
                        </div>
               ...
               <div className="pt-6 border-t border-white/5 flex justify-between items-center">
                  <div className="flex items-center gap-2 text-slate-500">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Prazo: {goal.deadline}</span>
                  </div>
          ...
          >
             ...
             <h4 className="text-xl font-black text-white">Adicionar Objetivo</h4>
              ...
              <div key={goal.id} className={`bg-surface-dark/40 border border-white/5 p-8 rounded-[2rem] space-y-6 group hover:border-white/10 transition-all ${goal.completed ? 'opacity-60' : ''}`}>
                 <div className="flex justify-between items-start">
                    <div className="space-y-1">
                       <h3 className={`text-xl font-black transition-all ${goal.completed ? 'text-slate-500 line-through' : 'text-white'}`}>{goal.title}</h3>
                       <span className="text-[10px] font-black text-primary uppercase tracking-widest">{goal.category}</span>
                    </div>
                 ...
                 <button className={`w-full py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${goal.completed ? 'bg-white/5 text-slate-500' : 'bg-primary text-black hover:scale-[1.02] shadow-lg shadow-primary/20'}`}>
